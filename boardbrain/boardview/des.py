...
# Minimal DES implementation (ported from dhuertas/DES, MIT).
...
def _permute(block: int, table: list[int], nbits: int) -> int:
    out = 0
    for t in table:
        out = (out << 1) | ((block >> (nbits - t)) & 1)
    return out


def _left_rotate(val: int, shift: int, bits: int) -> int:
...
def _sbox_substitution(val: int) -> int:
    out = 0
    for i in range(8):
        block = (val >> (42 - i * 6)) & 0x3F
...
def des_decrypt_block(block: int, key: int) -> int:
    ...
    # initial permutation
    ip = _permute(block, IP, 64)
    l = (ip >> 32) & 0xFFFFFFFF
    r = ip & 0xFFFFFFFF
    for i in range(15, -1, -1):
        l, r = r, l ^ _feistel(r, subkeys[i])
    preoutput = (r << 32) | l
    return _permute(preoutput, FP, 64)
